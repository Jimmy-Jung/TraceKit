...
TraceKit는 외부 모니터링/분석 서비스와 연동하기 위한 별도 모듈을 제공합니다.
...
| 모듈 | 서비스 | 용도 |
|-----|-------|------|
| TraceKitSentry | Sentry | 에러 모니터링, 이슈 추적 |
| TraceKitDatadog | Datadog | 로그 분석, 메트릭 |
| TraceKitFirebase | Firebase Crashlytics | 크래시 리포팅 |
...
let workspace = Workspace(
    ...
    projects: [
        "Projects/MyApp",
        "Projects/TraceKit",
        "Projects/TraceKitSentry",     // Sentry 사용 시
        "Projects/TraceKitDatadog",    // Datadog 사용 시
        "Projects/TraceKitFirebase"    // Firebase 사용 시
    ]
)
    ...
    targets: [
        .target(
            ...
            dependencies: [
                ...
                .project(target: "TraceKitSentry", path: "../TraceKitSentry"),
                // ...
            ]
        )
    ]
...
1. Sentry SDK 추가 (SPM)
2. TraceKitSentry 모듈 활성화

### 초기화

```swift
import TraceKit
import TraceKitSentry
...
2. TraceKitDatadog 모듈 활성화

### 초기화

```swift
import TraceKit
import TraceKitDatadog
import DatadogCore
...
struct MyApp: App {
    ...
    @TraceKitActor
    func setupTraceKit() async {
        let crashlyticsDestination = CrashlyticsTraceDestination(
            minLevel: .info
        )
...
func setupProductionTraceKit() async {
    // Sentry - 에러 추적
    let sentryDest = SentryTraceDestination(
        dsn: "https://xxx@sentry.io/123",
        minLevel: .warning,
        eventMinLevel: .error,
        environment: "production"
    ...
    // Datadog - 로그 분석
    let datadogDest = DatadogTraceDestination(
        clientToken: "pub_xxx",
        environment: "production",
        serviceName: "my-app",
        minLevel: .info
    ...
    // Crashlytics - 크래시 리포팅
    let crashlyticsDest = CrashlyticsTraceDestination(
        minLevel: .info
    )
...
func setupTraceKit(environment: Environment) async {
    ...
    case .production:
        ...
        let crashlytics = CrashlyticsTraceDestination(minLevel: .info)
