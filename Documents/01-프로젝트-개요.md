...
TraceKit는 iOS 애플리케이션을 위한 유연하고 확장 가능한 로깅 프레임워크입니다. Swift의 Actor를 활용한 스레드 안전한 설계와 다양한 출력 대상 지원이 특징입니다.

## 주요 기능

### 1. 다중 출력 대상 (Multi-Destination)

하나의 로그를 여러 곳에 동시에 출력할 수 있습니다. 기본 제공되는 Destination과 프로토콜을 통해 확장 가능한 Integration을 지원합니다:

| Destination | 설명 | 유형 | 용도 |
|-------------|------|------|------|
| Console | 콘솔 출력 | Built-in | 개발 중 디버깅 |
| OSLog | Apple 시스템 로그 | Built-in | Instruments 연동, 시스템 콘솔 |
| File | 파일 저장 | Built-in | 오프라인 분석, 로그 업로드 |
| Firebase | Crashlytics/Analytics | Integration | 크래시 리포팅, 분석 (Demo 참조) |
| Sentry | 에러 모니터링 | Integration | 크래시 추적 (사용자 구현) |
| Datadog | 로그 분석 | Integration | 대규모 로그 분석 (사용자 구현) |
...
let parentId = await TraceKit.shared.startSpan(name: "전체 작업")
    let childId = await TraceKit.shared.startSpan(name: "하위 작업", parentId: parentId)
    // ...
...
TraceKit는 단일 코어 모듈로 구성되며, 외부 의존성 없이 가볍게 동작합니다.
...
### 확장성
TraceKit는 프로토콜 기반 설계를 통해 다양한 외부 서비스를 쉽게 연동할 수 있습니다. 데모 앱(`TraceKitDemo`)에서 다음과 같은 연동 예시를 확인할 수 있습니다:

- **Firebase Crashlytics**: `FirebaseCrashlyticsTraceDestination`
- **Firebase Analytics**: `FirebaseAnalyticsTraceDestination`
- **Integration Examples**: Sentry, Datadog 등 서드파티 서비스 연동 가능
...
TraceKit는 `await` 없이도 사용할 수 있는 Fire-and-Forget API를 제공합니다:
...
| API 유형 | 사용 시점 | 예시 |
|---------|---------|-----|
| 동기 | 일반 로깅, UI 이벤트 | `TraceKit.info("...")` |
| 비동기 | 완료 대기, 테스트 | `await TraceKit.shared.info("...")` |
...
- `TraceDestination`: 로그 출력 대상
- `TraceFormatter`: 로그 포맷터
- `TraceSanitizer`: 민감정보 정제기
