...
`CrashLogPreserver` 기능을 테스트할 수 있는 화면입니다. 크래시 발생 시 로그를 보존하고 복구하는 전체 플로우를 확인할 수 있습니다.
...
#### CrashLogPreserver 통합

TraceKitDemo는 `TraceKitSetup`에서 `CrashLogPreserver`를 초기화합니다.
...
|------------|------|-----------|
| OSTraceDestination | Xcode 콘솔, Console.app | PrettyLogFormatter.verbose |
| FileTraceDestination | 디바이스에 로그 파일 저장 | JSONLogFormatter |
