...
Logger는 Actor 기반의 스레드 안전한 로깅 시스템입니다. 파사드 패턴과 빌더 패턴을 활용하여 복잡한 내부 구현을 숨기고 쉬운 인터페이스를 제공합니다.
...
flowchart TB
    ...
    subgraph Destinations["📤 Destinations"]
        Console["ConsoleLogDestination"]
        OSLog["OSLogDestination"]
        File["FileLogDestination"]
        External["External Destinations"]
    end
    
    subgraph External["🔌 External Modules"]
        Sentry["SentryLogDestination"]
        Datadog["DatadogLogDestination"]
        Firebase["CrashlyticsLogDestination"]
    end
...
```
Projects/Logger/Sources/Logger.swift
```
...
API 유형:
- 동기 API: `Logger.info("...")` - await 없이 사용 (Fire-and-Forget)
- 비동기 API: `await Logger.shared.info("...")` - 완료 대기 가능

### 2. LoggerBuilder (빌더)

복잡한 Logger 설정을 단순화합니다.

```
Projects/Logger/Sources/LoggerBuilder.swift
```
...
### 3. LogDestination (프로토콜)

모든 출력 대상이 구현해야 하는 프로토콜입니다.

```
Projects/Logger/Sources/Core/LogDestination.swift
```
...
```
Projects/Logger/Sources/Core/LogMessage.swift
```
...
```
Projects/Logger/Sources/
├── Logger.swift              # 메인 Logger 클래스
├── LoggerActor.swift         # @LoggerActor 정의
├── LoggerBuilder.swift       # 빌더
│
├── Core/                     # 핵심 타입
│   ├── LogLevel.swift        # 로그 레벨 enum
│   ├── LogMessage.swift      # 로그 메시지 값 객체
│   ├── LogDestination.swift  # Destination 프로토콜
...
Logger 전용 Actor로, 모든 Logger 관련 작업이 동일한 격리 컨텍스트에서 실행됩니다.
...
```swift
public actor MyCustomDestination: LogDestination {
...
- Core Logger는 외부 의존성이 없습니다
- 외부 연동 모듈은 Core Logger에만 의존합니다
