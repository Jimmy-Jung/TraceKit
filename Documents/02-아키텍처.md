...
flowchart TB
    ...
    subgraph ExternalIntegrations["🔌 Integration Examples"]
        FirebaseCrashlytics["FirebaseCrashlyticsTraceDestination"]
        FirebaseAnalytics["FirebaseAnalyticsTraceDestination"]
    ...
    class FirebaseCrashlytics,FirebaseAnalytics,Custom external
```
...
│   ├── TraceFormatter.swift    # Formatter 프로토콜
│   ├── TraceSanitizer.swift    # Sanitizer 프로토콜
...
public actor MyCustomDestination: TraceDestination {
    public nonisolated let identifier = "custom"
...
```swift
public struct MyFormatter: TraceFormatter {
...
```swift
public struct MySecuritySanitizer: TraceSanitizer {
    public var isEnabled: Bool = true
...
flowchart BT
    subgraph Integrations["통합 예시 (사용자 구현)"]
        FirebaseDest["Firebase Destinations"]
        CustomDest["Custom Destinations"]
    end
    
    subgraph Core["TraceKit (Core)"]
        TraceKit["TraceKit"]
        Protocols["TraceDestination\nTraceFormatter\nTraceSanitizer"]
    end
    
    FirebaseDest --> Protocols
    CustomDest --> Protocols
    Protocols -.정의.-> TraceKit
    
    classDef core fill:#8b5cf6,stroke:#a78bfa,color:#ffffff
    classDef protocol fill:#3b82f6,stroke:#60a5fa,color:#ffffff
    classDef ext fill:#10b981,stroke:#34d399,color:#ffffff
    
    class TraceKit core
    class Protocols protocol
    class FirebaseDest,CustomDest ext
```

- Core TraceKit는 외부 의존성이 없습니다
- 사용자는 `TraceDestination` 프로토콜을 구현하여 커스텀 연동을 추가할 수 있습니다
- 데모 앱(`TraceKitDemo`)에서 Firebase 연동 예시를 확인할 수 있습니다
