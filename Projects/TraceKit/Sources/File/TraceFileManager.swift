...
public actor TraceFileManager {
    /// 로그 파일 기본 디렉토리
    private let baseDirectory: URL

    /// 보관 정책
    private let retentionPolicy: TraceFileRetentionPolicy
    ...
    public init(
        ...
        if let baseDirectory = baseDirectory {
            ...
            // 플랫폼별 적절한 로그 디렉토리 설정
                ...
                let bundleId = Bundle.main.bundleIdentifier ?? "com.logger"
                self.baseDirectory = logsDir.appendingPathComponent(bundleId, isDirectory: true)
            #elseif os(watchOS)
                // watchOS: 제한된 저장 공간, 문서 디렉토리 사용
                let docsDir = fm.urls(for: .documentDirectory, in: .userDomainMask).first!
                self.baseDirectory = docsDir.appendingPathComponent("Logs", isDirectory: true)
            #else
                // 기타 플랫폼: Caches 디렉토리
                let cachesDir = fm.urls(for: .cachesDirectory, in: .userDomainMask).first!
                self.baseDirectory = cachesDir.appendingPathComponent("Logs", isDirectory: true)
