...
public struct TraceKitConfiguration: Sendable, Equatable {
    /// 최소 로그 레벨
    ...
    /// 다른 설정과 병합 (other가 우선)
    public func merged(with other: TraceKitConfiguration) -> TraceKitConfiguration {
    ...
    /// 기본 설정
    public static let `default` = TraceKitConfiguration()

    /// 디버그용 설정
    public static let debug = TraceKitConfiguration(
        minLevel: .verbose,
    ...
    /// 프로덕션용 설정
    public static let production = TraceKitConfiguration(
        minLevel: .info,
