...
public actor FileTraceDestination: TraceDestination {
    ...
    /// 파일 관리자
    private let fileManager: TraceFileManager
    ...
    public init(
        ...
        isEnabled: Bool = true,
        formatter: TraceFormatter = JSONTraceFormatter(),
        fileManager: TraceFileManager,
        bufferSize: Int = 10
    ) {
    ...
    public func flush(_ messages: [TraceMessage]) async {
    ...
    private func flushBuffer() async {
        ...
        do {
            ...
            // 파일 쓰기 실패 시 콘솔에 출력
            print("[FileTraceDestination] Failed to write logs: \(error)")
        }
    }
