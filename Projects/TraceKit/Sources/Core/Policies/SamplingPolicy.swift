...
public struct SamplingPolicy: Sendable, Equatable {
    ...
    /// 레벨별 샘플링 비율
    public let ratesByLevel: [TraceLevel: Double]

    /// 카테고리별 샘플링 비율
    public let ratesByCategory: [String: Double]

    /// 항상 포함할 레벨 (샘플링 무시)
    public let alwaysIncludeLevels: Set<TraceLevel>

    public init(
        defaultRate: Double = 1.0,
        ratesByLevel: [TraceLevel: Double] = [:],
        ratesByCategory: [String: Double] = [:],
        alwaysIncludeLevels: Set<TraceLevel> = [.error, .fatal]
    ) {
    ...
    public func rate(for message: TraceMessage) -> Double {
        // 항상 포함할 레벨은 100%
    ...
    public static let debug = SamplingPolicy(
        defaultRate: 1.0,
        alwaysIncludeLevels: Set(TraceLevel.allCases)
    )
