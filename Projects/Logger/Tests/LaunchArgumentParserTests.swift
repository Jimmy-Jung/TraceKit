...
struct LaunchArgumentParserTests {
    ...
    @Test("logLevel 인자 파싱", arguments: [
        (["app", "-logLevel", "DEBUG"], LogLevel.debug),
        (["app", "-logLevel", "INFO"], LogLevel.info),
        (["app", "-logLevel", "WARNING"], LogLevel.warning),
        (["app", "-logLevel", "ERROR"], LogLevel.error),
        (["app", "-logLevel", "FATAL"], LogLevel.fatal)
    ])
    func parseLogLevel(arguments: [String], expectedLevel: LogLevel) {
        // When
