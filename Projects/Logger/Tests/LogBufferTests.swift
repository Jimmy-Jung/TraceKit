...
struct LogBufferTests {
    ...
    func appendMessage() async {
        // Given
        let buffer = LogBuffer(policy: .default)
    ...
    func autoFlushOnMaxSize() async {
        // Given
        let policy = LogBufferPolicy(maxSize: 3, flushInterval: 0, flushOnLevel: nil)
        let buffer = LogBuffer(policy: policy)
